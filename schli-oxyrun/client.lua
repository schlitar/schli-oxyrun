...
function CreateBlip()
	...
	if OxyRun then
		...
		QBCore.Functions.Notify("GPS'in güncellendi, konuma git!", "error")
		SetNewWaypoint(OxyDropOffs[rnd]['coords']["x"],OxyDropOffs[rnd]['coords']["y"],OxyDropOffs[rnd]['coords']["z"])
	end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			QBCore.Functions.Notify('Dağıtım noktasına çok yakınsın!')
		end
		toolong = toolong - 1
		if toolong < 0 then
		...
		if dstcheck < 2.0 and pedCreated then
			...
			local crds = GetEntityCoords(deliveryPed)
			QBCore.Functions.DrawText3D(crds["x"],crds["y"],crds["z"], "[E] İlacı Ver")  
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			QBCore.Functions.DrawText3D(pillWorker['coords']["x"],pillWorker['coords']["y"],pillWorker['coords']["z"], "[E] $2,000 - İlaç Dağıtım") 
